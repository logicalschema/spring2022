...
first_model <- lm(rate_offers ~ coursepassrate + elaprof + mathprof + surveysafety, data=schools_train)
summary(first_model)
...
ggplot(schools_test, aes(x=prediction, y=rate_offers)) + geom_point() + geom_abline(color="blue")
...
(rho <- cor(schools_test$prediction, schools_test$rate_offers))
